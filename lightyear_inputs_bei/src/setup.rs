...
use bevy_replicon::shared::replication::RemoteEntity;
...
impl InputRegistryPlugin {
    pub(crate) fn mirror_action_of_for_replication<C: Component>(
        ...
        remote_entities: Query<&RemoteEntity>,
    ...
    pub(crate) fn add_action_of_replicate<C: Component>(
        ...
        action: Query<&ActionOf<C>, (With<NetworkActionOf<C>>, Without<Remote>, Without<PreSpawned>)>,
        mut commands: Commands,
    ) {
...
fn resolve_remote_entity<'a>(
    ...
    if let Ok(remote_entity) = remote_entities.get(local_entity) {
        return Some(remote_entity.0);
    }
...
fn resolve_remote_action_context<'a>(
    ...
    resolve_remote_entity(local_entity, remote_entities, entity_map, managers)
